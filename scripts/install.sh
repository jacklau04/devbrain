...
' "$md" > "$tmp" && mv "$tmp" "$md"
{
  ...
  printf 'To READ a page a search surfaces, pass its FULL `<project>/<page>` slug from the\n'
  printf 'output to `gbrain get "<project>/<page>" --fuzzy` — not the bare page name (the\n'
  printf 'brain is one namespace, so a bare slug is `page_not_found`), and do not pipe the\n'
  printf 'read through `2>/dev/null`, which hides gbrain'\''s own "Did you mean" fix-hints.\n\n'
...
if want codex; then
  if [ "${codex_hooks_enabled:-0}" = 1 ]; then
    echo "  Codex: hooks feature enabled — restart Codex, then review/trust devbrain hooks with /hooks when prompted"
  else
    echo "  Codex: enable hooks yourself ('codex features enable hooks'), then restart Codex and trust them with /hooks"
  fi
fi
